...
const BannerHome = () => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            animationFrame = requestAnimationFrame(handleNext);
        }, 3000);
    ...
    useEffect(() => {
        if (isAnimating) {
            const timeout = setTimeout(() => setIsAnimating(false), 500); // Adjust based on transition duration
                    ...
                    <div key={data.id + "bannerHome" + index} className='min-w-full min-h-[450px] lg:min-h-full overflow-hidden relative group transition-all' style={{ transform: `translateX(-${currentImage * 100}%)` }}>
                        ...
                        <div className='container mx-auto'>
                            <div className='w-full absolute bottom-0 max-w-md px-3'>
                                <h2 className='font-bold text-2xl lg:text-4xl text-white drop-shadow-2xl'>{data.title ? data.title : data.name}</h2>
